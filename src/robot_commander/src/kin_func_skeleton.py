...
def rotation_3d(omega, theta):
    ...
    detOmega= np.linalg.norm(omega)
                          
    rot = np.eye(3) +  np.divide(omega_hat,(detOmega))*np.sin(theta*detOmega) + np.divide(np.dot(omega_hat,omega_hat),(detOmega**2))*(1-np.cos(theta*detOmega))
...
def homog_3d(xi, theta):
    ...
    om = np.array([xi[3],xi[4],xi[5]])
    if ((om[0] ==0 ) and (om[1] == 0) and (om[2] == 0)):
        g = np.array([[1, 0, 0, v[0]*theta], [0, 1, 0, v[1]*theta], [0, 0, 1, v[2]*theta], [0, 0,0,1]])  
    else: 
                          
        Rot= rotation_3d(om,theta)
        first = np.eye(3)  - Rot
        omHat = skew_3d(om)

        second = np.dot(omHat, v)
        firstSecond = np.matmul(first,second)
        firstSecond = firstSecond.reshape(-1,1)
        detOmega= np.linalg.norm(om)
        omT = om.reshape(-1,1)

        vt=v.reshape(-1,1)
        third = np.matmul(omT*om,vt) *theta

        term = (1/detOmega**2)* (firstSecond  + third)
        g =Rot
        gBot= np.array([0,0,0])
        g = np.vstack((g,gBot))
        term = np.vstack((term,np.array([1])))
        g = np.append(g, term, axis=1)
    return g
    # YOUR CODE HERE
...
def prod_exp(xi, theta):
    ...
    homog_3d(xi[:, 0], theta[0])
    for i in range(N):
        gDum= homog_3d(xi[:,i] , theta[i])
        g = np.matmul(g,gDum)
